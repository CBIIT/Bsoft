...
# Created: 20010210 	Modified: 20170822
#----------------------------------------------------------#
...
my @tags = qw/Action Parameter Input Output Example Prepar Select Final Configur Creation/;
...
#				} elsif ( $line =~ /^Actions/ || $line =~ /^Parameters/ ||
#						$line =~ /^Input/ || $line =~ /^Output/ || $line =~ /^Example/ ) {
				} elsif ( grep { $line =~ /^$_/ } @tags ) {
					if ( $state > 2 ) {
...
sub close_html_file {
	...
	print $file <<END;
<HR WIDTH=\"100%\"><BR>
Generated by $prog on $thetime <BR>
<HR WIDTH=\"100%\"><BR>
